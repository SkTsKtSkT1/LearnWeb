...
Address::ptr
Address::LookupAny(const std::string &host, int family, int type, int protocol) {
...
std::shared_ptr<IPAddress> Address::LookupAnyIPAddress(const std::string &host, int family, int type, int protocol) {
    ...
    if(Lookup(result, host, family, type, protocol)){
        for(auto& i : result){
            //SKT_LOG_DEBUG(g_logger) << i->toString();
...
bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host, int family, int type, int protocol) {
    ...
    if(error){
        SKT_LOG_ERROR(g_logger) << " Address::Lookup getaddress(" << host << ", " << family << ", " << type << ") err="
    ...
    while(next){
        ...
        //SKT_LOG_DEBUG(g_logger) << ((sockaddr_in*)next->ai_addr)->sin_addr.s_addr;
        next = next->ai_next;
    }
...
Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>> &result, int family) {
    ...
    if(getifaddrs(&results) != 0){
        SKT_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses getifaddrs " << " err=" << errno << " errstr=" << strerror(errno);
        return false;
    }
    try{
        ...
        SKT_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
bool Address::operator<(const skt::Address &rhs) const {
...
IPAddress::ptr IPAddress::Create(const char *address, uint16_t port) {
    ...
    if(error == EAI_NONAME){
        SKT_LOG_ERROR(g_logger) << "IPAddress::Create(" << address << ", " << port << ") error=" << error
...
IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port) {
    ...
    if(result <= 0){
        SKT_LOG_ERROR(g_logger) << "IPv4Address::Create(" << address << ", "
...
std::ostream& IPv6Address::insert(std::ostream &os) const {
    os << "[";
    uint16_t *addr;
    addr = (uint16_t*)m_addr.sin6_addr.s6_addr;
