...
HttpConnection::~HttpConnection(){
    SKT_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResult::ptr HttpConnection::DoRequest(skt::http::HttpMethod method, const std::string &url, uint64_t timeout_ms,
                                          const std::map<std::string, std::string> &headers, const std::string &body) {
    Uri::ptr uri = Uri::Create(url);
    if(!uri){
        return std::make_shared<HttpResult>((int)HttpResult::Error::INVALID_URL, nullptr, "invalid url:" + url);
...
HttpResult::ptr HttpConnection::DoRequest(skt::http::HttpMethod method, Uri::ptr uri, uint64_t timeout_ms,
                                          const std::map<std::string, std::string> &headers, const std::string &body) {
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = skt::GetCurrentMs();
    ...
    if(!ptr){
        ...
        if(!addr){
            SKT_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock){
            SKT_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)){
            SKT_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(skt::http::HttpConnection *ptr, skt::http::HttpConnectionPool *pool) {
    ++ptr->m_request;
    if(!ptr->isConnected() || ((ptr->m_createTime + pool->m_maxAliveTime) >= skt::GetCurrentMs())
...
HttpResult::ptr HttpConnectionPool::doRequest(skt::http::HttpMethod method, const std::string &url, uint64_t timeout_ms,
...
HttpResult::ptr HttpConnectionPool::doRequest(skt::http::HttpMethod method, Uri::ptr uri, uint64_t timeout_ms,
                                              const std::map<std::string, std::string> &headers,
                                              const std::string &body) {
    std::stringstream ss;
    ss << uri->getPath()
