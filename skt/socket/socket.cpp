...
Socket::ptr Socket::CreateTCP(skt::Address::ptr address){
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(skt::Address::ptr address){
...
bool Socket::getOption(int level, int option, void *result, size_t *len) {
    ...
    if(rt){
        SKT_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock  << " level=" << level << " option=" << option
...
bool Socket::setOption(int level, int option, const void *result, size_t len) {
    ...
    if(rt){
        SKT_LOG_DEBUG(g_logger) << "setOption sock=" << m_sock  << " level=" << level << " option=" << option
                                << " errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
    return true;
...
Socket::ptr Socket::accept() {
    ...
    if(newsock == -1){
        SKT_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno=" << errno << " errstr=" << strerror(errno);
        return nullptr;
    }
...
bool Socket::bind(const Address::ptr addr) {
    if(!isValid()){
        newSock();
        if(SKT_UNLIKELY(!isValid())){
            return false;
        }
    }
    if(SKT_UNLIKELY(addr->getFamily() != m_family)){
        SKT_LOG_ERROR(g_logger) << "bind sock.family(" << m_family << ") addr.family(" << addr->getFamily()
             << ") not equal, addr=" << addr->toString();
        return false;
    }

    if(::bind(m_sock, addr->getAddr(), addr->getAddrLen())){
        SKT_LOG_ERROR(g_logger) << "bind errno" << errno
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if(SKT_UNLIKELY(addr->getFamily() != m_family)){
        SKT_LOG_ERROR(g_logger) << "connect sock.family(" << m_family << ") addr.family(" << addr->getFamily()
                                << ") not equal, addr=" << addr->toString();
        return false;
    }
    if(timeout_ms == (uint64_t)-1){
        if(::connect(m_sock, addr->getAddr(), addr->getAddrLen())){
            SKT_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
...
bool Socket::listen(int backlog) {
    if(SKT_UNLIKELY(!isValid())){
        SKT_LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
    if(::listen(m_sock, backlog)){
        SKT_LOG_ERROR(g_logger) << "listen error errno=" << errno
            << "errstr=" << strerror(errno);
        return false;
    }
    return true;
...
Address::ptr Socket::getRemoteAddress() {
    ...
    if(getpeername(m_sock, result->getAddr(), &addrlen)){
        SKT_LOG_ERROR(g_logger) << "getpeername error sock=" << m_sock << " errno=" << errno
            << " errstr=" << strerror(errno);
...
Address::ptr Socket::getLocalAddress() {
    ...
    if(getsockname(m_sock, result->getAddr(), &addrlen)){
        SKT_LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock << " errno=" << errno
                                << " errstr=" << strerror(errno);
        return Address::ptr(new UnknowAddress(m_family));
    }
    if(m_family == AF_UNIX){
...
// output the information of the socket
...
void Socket::newSock() {
    ...
    if(SKT_LIKELY(m_sock != -1)){
        initSock();
    }else{
        SKT_LOG_ERROR(g_logger) << "socket(" << m_family << ", " << m_type << ", "
            << m_type << ", " << m_protocol << ") errno=" << errno << " errstr=" << strerror(errno);
    }
}
}