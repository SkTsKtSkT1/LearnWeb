...
void test_request(){
    skt::http::HttpRequestParser parser;
    std::string tmp = test_request_data;
    size_t s = parser.excute(&tmp[0], tmp.size());
    SKT_LOG_ERROR(g_logger) << "execute rt=" << s
    ...
    SKT_LOG_INFO(g_logger) << parser.getData()->toString();
    SKT_LOG_INFO(g_logger) << tmp;
}
...
void test_response(){
    skt::http::HttpResponseParser parser;
